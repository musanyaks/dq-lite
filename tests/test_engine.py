...
class TestValidate:
    ...
    def test_not_null_expectation_passes(self):
        ...
        result = validate(df, expectations=[expect("id").not_null()])
        assert result.success is True
    ...
    def test_between_expectation_passes(self):
        ...
        result = validate(df, expectations=[expect("age").between(0, 120)])
        assert result.success is True
    ...
    def test_unique_expectation_passes(self):
        ...
        result = validate(df, expectations=[expect("email").unique()])
        assert result.success is True
    ...
    def test_in_set_expectation_passes(self):
        ...
        result = validate(
            df, expectations=[expect("status").in_set(["active", "inactive", "pending"])]
        )
        assert result.success is True

    def test_in_set_expectation_fails(self):
        df = pd.DataFrame({"status": ["active", "deleted", "active"]})
        result = validate(df, expectations=[expect("status").in_set(["active", "inactive"])])
        assert result.success is False
        assert result.results[0].unexpected_count == 1

    def test_regex_expectation_passes(self):
        df = pd.DataFrame({"email": ["a@b.com", "c@d.org"]})
        result = validate(df, expectations=[expect("email").matches_regex(r".+@.+\..+")])
        assert result.success is True
    ...
    def test_row_count_expectation(self):
        ...
        result = validate(df, expectations=[expect_table().row_count().between(3, 10)])
        assert result.success is True

    def test_row_count_expectation_fails(self):
        df = pd.DataFrame({"a": [1]})
        result = validate(df, expectations=[expect_table().row_count().greater_than(5)])
        assert result.success is False
    ...
    def test_statistics_present(self):
        df = pd.DataFrame({"a": [1, 2, 3]})
        result = validate(df, expectations=[expect("a").not_null()])
